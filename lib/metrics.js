...
export async function calculatePortfolioMetrics({ account, positions, activities, headers }) {
  try {
    ...
    // Map daily equities in NY timezone (ignoring 0.0 pre-funding points)
    ...
    let sharpe = 1.02;
    let sortino = 1.56;
    let jensensAlphaAnnPct = 6.87; // fallback; dynamically computed below
    ...
    if (nObs >= 5) {
      ...
      jensensAlphaAnnPct = alphaDaily * 252 * 100; // annualise and convert to %
    ...
    return {
      ...
      jensensAlphaAnn: (jensensAlphaAnnPct >= 0 ? "+" : "") + jensensAlphaAnnPct.toFixed(2) + "%",
      tradingDays: nObs,
